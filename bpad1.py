...
def swap_token(token, from_token, to_token, amount):
    while True:  # Loop terus menerus sampai swap berhasil
        log(f"Mengirim permintaan swap: {amount:.6f} {from_token} -> {to_token}...")
        ...
        if response.status_code == 200:
            log(f"[SUCCESS] Swap berhasil: {amount:.6f} {from_token} -> {to_token}", "SUCCESS")
...
if __name__ == "__main__":
    log("Memulai skrip BPAD to tICE...")
    token = load_token()
    if not token:
    ...
    while True:
        amount = round(random.uniform(0.001, 0.009), 6)  # Nilai acak antara 0.001 dan 0.009
        swap_token(token, "BPAD", "tICE", amount)
